...
const BUILDINGS = {//function context == point
	goldFactory : {
		...
		info() {
			return "Production: " + displayNumber(this.depth * this.map.level ** 2 * ((this.level || 0) + 1) * (game.skills.greed ? 10 : 1)) + "/s"
		},
		build() {
			game.production.gold += this.depth * this.map.level ** 2 * ((this.level || 0) + 1) * (game.skills.greed ? 10 : 1)
		},
		destroy() {
			game.production.gold -= this.depth * this.map.level ** 2 * ((this.level || 0) + 1) * (game.skills.greed ? 10 : 1)
		},
	...
	scienceLab : {
		...
		info() {
			return "Production: " + displayNumber(this.depth * this.map.level ** 2 / 1e5 * ((this.level || 0) + 1) * ((this.level || 0) + 1))
		},
		build() {
			game.production.science += this.depth * this.map.level ** 2 / 1e5 * ((this.level || 0) + 1)
		},
		destroy() {
			game.production.science -= this.depth * this.map.level ** 2 / 1e5 * ((this.level || 0) + 1)
		},
