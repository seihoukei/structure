...
const MapTab = Template({
	...
	updateGrowth() {
		this.displayGrowth.map(x => {
			...
			x.dvValue.innerText = displayNumber(game.growth[x.name]) + (game.real.multi[x.name] != 1?" × "+displayNumber(game.real.multi[x.name]) + (x.name == "power" && game.real.multi[x.name] == game.world.stats.powerCap?" (CAP)":"")+" = "+displayNumber(game.real.growth[x.name]):"")
